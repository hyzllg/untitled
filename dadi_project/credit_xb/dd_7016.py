...
def jh_main(environment,loanAmount,periods):
    ...
    ora_conf = get_yaml_data(f'{path}/conf/oracle/xshx_oracle.yaml')
    res_url = get_yaml_data(f'{path}/conf/url_res/api_url_jh.yaml')
    res_data = get_yaml_data(f'{path}/conf/loan_res/jh_res_data.yaml')
    #日志
    log = my_log.Log()
    #获取数据库配置
    oracle_conf = get_oracle_conf(ora_conf,environment)
