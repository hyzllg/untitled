...
class CLAIM_RESULT:
    ...
    def Payamt(self,datetime1):
        #未还本金
        a = Collect.sql_cha(self.cursor,"select sum(paycorpusamt),sum(actualpaycorpusamt) from acct_payment_schedule s  where s.objectno = '{}'".format(self.loanNo))
        corpusamt = a[0][0] - a[0][1]
        #逾期本金
        #先查一下最早逾期期次
        n = Collect.sql_cha(self.cursor,f"select seqid from acct_payment_schedule s  where s.objectno = '{self.loanNo}'and status = '12'")[0][0]
        a = Collect.sql_cha(self.cursor,f"select sum(paycorpusamt),sum(actualpaycorpusamt) from acct_payment_schedule s  where s.objectno = '{self.loanNo}'and seqid in ('{n}','{n+1}','{n+2}')")
        yq_corpusamt = a[0][0] - a[0][1]
        #正常本金
        zc_corpusamt = corpusamt - yq_corpusamt

        #利息 正常本金(1683.86)*利率0.062/360*18 + 逾期期数未还利息
        a = Collect.sql_cha(self.cursor,f"select sum(payinteamt),sum(actualpayinteamt) from acct_payment_schedule s  where s.objectno = '{self.loanNo}'and seqid in ('{n}','{n+1}','{n+2}')")
        #逾期整期未还利息
        yq_inteamt = a[0][0] - a[0][1]
        #理赔日前不足整期利息
        datetime2 = Collect.sql_cha(self.cursor,f"select paydate from acct_payment_schedule s  where s.objectno = '{self.loanNo}' and seqid = '{n+2}'")[0][0]
        days = self.Caltime(datetime1,datetime2)
        payinteamt = round(zc_corpusamt * 0.062/360 * days + yq_inteamt,2)

        #罚息
        a = Collect.sql_cha(self.cursor,f"select sum(payfineamt),sum(actualfineamt) from acct_payment_schedule s  where s.objectno =  '{self.loanNo}'and seqid in ('{n}','{n+1}','{n+2}')")
        fineamt = a[0][0] - a[0][1]
    ...
    def data(self,productid,datetime0,datatime3,payamt,TASK_ID):#productid
        ...
        a = Collect.sql_cha(self.cursor,"select a.customername,a.accountno,af.result_seq_no,a.putoutno from acct_loan a inner join acct_fund_apply af on a.apply_no = af.apply_no where serialno = '{}'".format(self.loanNo))[0]
...
def main(a):
    ...
    for loanNo in loanNo:
        ...
        #最早逾期期次还款日
        try:
            datetime0 = Collect.sql_cha(cursor,f"select paydate from acct_payment_schedule s  where s.objectno = '{loanNo}'and status = '12'")[0][0]
        except IndexError:
